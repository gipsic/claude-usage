...
### Fixed
- **The reset time flipped to "inferred" every time the desktop cache was newer than the API poll.** The cache is imported every 30 s and its newest sample never carries a reset time, while the endpoint is polled every 3 min and does; whenever the cache sample was the fresher of the two the card lost the exact `resets_at` and fell back to the inferred one - with live data in hand. The newest sample still supplies the percentage, and a reported reset time that is still in the future at that sample's time is carried forward with it.
- Request counts under 100 printed with a decimal (`60.0` requests in the Session history table). Integers now print as integers.
